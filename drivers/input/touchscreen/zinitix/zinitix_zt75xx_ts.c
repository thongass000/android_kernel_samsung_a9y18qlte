...
#define DEF_MIS_CAL_SPEC_MIN 20
#define DEF_MIS_CAL_SPEC_MAX 180
...
static bool ts_read_coord(struct bt532_ts_info *info)
{
	...
	if((info->pdata->support_lpm_mode)&&(zinitix_bit_test(info->touch_info.status, BIT_GESTURE))){
		...
		if (zinitix_bit_test(lpm_mode_reg.data, BIT_EVENT_SPAY)) {
			...
			input_report_key(info->input_dev, KEY_BLACK_UI_GESTURE, 0);
			input_sync(info->input_dev);
		} else if (zinitix_bit_test(lpm_mode_reg.data, BIT_EVENT_SINGLE_TAP)) {
			input_info(true, &client->dev, "%s: single tab\n", __func__);
...
static irqreturn_t bt532_touch_work(int irq, void *data)
{
	...
	if (IRQ_HANDLED == secure_filter_interrupt(info)) {
		...
		dev_dbg(&info->client->dev, "%s: secure interrupt handled\n", __func__);
	...
	for (i = 0; i < info->cap_info.multi_fingers; i++) {
		...
		if (zinitix_bit_test(info->touch_info.status, BIT_PALM_REJECT)) {
			...
			if (zinitix_bit_test(sub_status, SUB_BIT_PALM) == 0x80) {
				palm = 1;
			} else {
				palm = 0;
			}
...
static void bt532_ts_close(struct input_dev *dev)
{
	...
	if((info->pdata->support_lpm_mode) && (info->spay_enable || info->aod_enable) && (!info->prox_power_off)){
		...
		input_info(true, &misc_info->client->dev,
				"%s: write lpm_mode 0x%02x (spay:%d, aod:%d, single tab:%d)\n", __func__, info->lpm_mode,
...
static void singletap_enable(void *device_data)
{
	...
	temp = info->lpm_mode;
re_write:
	if (write_reg(info->client, ZT75XX_LPM_MODE_REG, temp) != I2C_SUCCESS)
		input_info(true, &misc_info->client->dev, "%s, fail lpm mode set\n", __func__);

	if (read_data(info->client, ZT75XX_LPM_MODE_REG, (u8 *)&lpm_mode_reg, 2) < 0)
		input_info(true, &client->dev, "%s: lpm_mode_reg read fail\n", __func__);

	input_info(true, &client->dev, "%s: lpm_mode_reg temp 0x%02x reg 0x%02x\n", __func__,
				temp, lpm_mode_reg.flag);

	if ((temp != lpm_mode_reg.flag) && (retry > 0)) {
		retry--;
		goto re_write;
	}
	write_cmd(info->client, 0x0B);
