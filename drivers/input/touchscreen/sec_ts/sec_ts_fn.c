		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < ts->tx_count; i++) {
		snprintf(temp, sizeof(temp), "\"TAMB_TX%02d\":\"%d\",",
				i, ts->ambient_tx[i]);
		strlcat(buffer, temp, sizeof(buf));
	}
...
static void set_factory_level(void *device_data)
{
	...
	ts->factory_position = sec->cmd_param[0] + 1;
...
*/
	data[0] = 0x81;
	data[1] = 0x02;
	rc = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_SELFTEST, data, 2);
	if (rc < 0)
		goto cm_gap_i2c_err;
	...
	/* set mutual type 0xFF */
	data[0] = 0xFF;
	rc = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_MUTU_RAW_TYPE, data, 1);
	if (rc < 0)
		goto cm_gap_i2c_err;

	/* set factory level to 0 */
	sec_ts_write_factory_level(ts, OFFSET_FW_NOSAVE);
...
static void run_pressure_filtered_strength_read_all(void *device_data)
{
	...
	/* dreamlite: right - center - left: pressure[2], pressure[1], pressure[0] */
 ...
 *
 * strength data save sequence @ TSP IC :    RIGHT -> CENTER -> LEFT
 * strength data display sequence @ TSP IC : LEFT  -> CENTER -> RIGHT
 */
